...
function renderSession() {
  ...
  const sessions = getSessionsForDay(state.selectedDay);

  // Haupteinheit für Rendering (abends oder einzige Session)
  let primarySession = getPrimarySessionForDay(state.selectedDay);
  let morningSession = sessions.find(s => s.timeOfDay === 'morning');

  // Load workout data
  if (state.editingPastDate === dateKey && state.editingPastData) {
    state.todayData = state.editingPastData;
  } else {
    state.todayData = loadWorkout(dateKey) || { date: dateKey, type: primarySession.type };
  }

  // WICHTIG: Für gespeicherte Workouts die Session aus den Daten rekonstruieren
  if (state.todayData.type && (state.todayData.exercises || state.todayData.run || state.todayData.completed)) {
    // Es gibt gespeicherte Daten - finde die passende Session im Pool
    const savedType = state.todayData.type;
    const matchingSession = Object.values(SESSIONS).find(s => s.type === savedType &&
      (savedType !== 'gym' || (state.todayData.exercises && Object.keys(state.todayData.exercises)[0] &&
       s.exercises && s.exercises.some(ex => ex.id === Object.keys(state.todayData.exercises)[0]))));

    if (matchingSession) {
      primarySession = matchingSession;
    }
  }
  ...
  if (morningSession && (morningSession.session.type === 'run' || morningSession.session.type === 'long')) {
    const m = morningSession.session, r = state.todayData.run || {}, d = (state.todayData.completed || isFuture || pastDayLocked) ? 'disabled' : '';
    html += `<div class="morning-session-container"><div class="gym-session-card"><div class="session-header"><div><div class="session-title-gym">${m.name}</div><div class="session-subtitle">${m.subtitle}</div><div style="margin-top:8px;"><span class="session-badge badge-${m.type}">${DAY_TYPE_LABELS[m.type]}</span></div></div></div><div class="run-form" style="padding:0 16px 16px;"><div class="run-field"><label>Distanz</label><input type="number" step="0.01" placeholder="${m.targetDistance}" inputmode="decimal" data-run="distance" value="${r.distance || ''}" ${d}/></div><div class="run-field"><label>Zeit (mm:ss)</label><input type="text" placeholder="z.B. 42:30" data-run="time" value="${r.time || ''}" ${d}/></div><div class="run-field"><label>Pace</label><div class="computed" id="computedPace">—</div></div><div class="run-field"><label>Ø Puls</label><input type="number" placeholder="${m.targetHR}" inputmode="numeric" data-run="hr" value="${r.hr || ''}" ${d}/></div><div class="run-field"><label>Kalorien</label><input type="number" placeholder="kcal" inputmode="numeric" data-run="kcal" value="${r.kcal || ''}" ${d}/></div><div class="run-field"><label>Kadenz</label><input type="number" placeholder="Schritte/min" inputmode="numeric" data-run="cadence" value="${r.cadence || ''}" ${d}/></div><div class="run-field run-field-full"><label>Notizen</label><input type="text" placeholder="Schmerzen, Gefühl" data-run="painLocation" value="${r.painLocation || ''}" ${d}/></div></div></div></div>`;
  }

  // Ab hier rendern wir die Haupteinheit (primarySession)
  const plan = primarySession;
  ...
  if (isFuture) {
    ...
    html += `
      <div class="save-bar gym-bar" style="justify-content:center;align-items:center;gap:10px;">
        ...
        ` : `
        <div style="display:flex;gap:6px;width:100%;">
          <button class="secondary" onclick="markIncomplete()"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-4.36"/></svg>Zurücksetzen</button>
          <button class="primary" onclick="enterEditMode()"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>Editieren</button>
        </div>
        `}
    ...
    html += `
      <div class="save-bar">
        ...
        ` : `
        <button class="primary" onclick="markComplete()"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>Einheit abschließen</button>
        `}
      </div>
    `;
  }
